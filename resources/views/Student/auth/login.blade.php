<div class="register">
    <div class="content">
        <div class="right">
            <p class="fs-1 header">{{__('Login for a Student Account')}}</p>
            @if (session('status'))
            ...
            @endif
            <form action="{{ route('student.auth.login') }}" method="POST">
                @csrf
                <div>
                ...
                <div class="footer">
                    ...
                    <p>{{__("Don't have an account?")}}
                        <a href="{{ route('student.auth.register') }}"> {{__('Sign up')}} </a>{{__('for free')}}
                    </p>
                </div>
